...
export function ProgramsCatalog({ programs }: ProgramsCatalogProps) {
  ...
  const categories = useMemo(
    () => ['All', ...new Set(programs.map((program) => program.category).filter(Boolean) as string[])],
    [programs]
  );
  ...
  const totalGoal = programs.reduce((sum, program) => sum + (program.goal || 0), 0);
  const totalRaised = programs.reduce((sum, program) => sum + (program.raised || 0), 0);
          ...
          <Container>
            <div className="max-w-4xl mx-auto text-center space-y-6 md:space-y-8">
              ...
              <p className="text-xl text-muted-foreground">
                Strategic initiatives bringing Bitcoin adoption and economic empowerment to African
                communities. Every donation directly supports practical, local impact.
              </p>
                    ...
                    <div className="p-6">
                      ...
                      <div className="mb-6">
                        <div className="flex justify-between text-sm mb-2">
                          <span className="font-medium">${(program.raised || 0).toLocaleString()}</span>
                          <span className="text-muted-foreground">of ${(program.goal || 0).toLocaleString()}</span>
                        </div>
                      ...
                      <div className="flex gap-3">
                        <Button className="flex-1" onClick={() => handleDonate(program.slug)}>
                          Donate
                        </Button>
          ...
          <Container>
            <div className="text-center max-w-3xl mx-auto space-y-6 md:space-y-8">
              ...
              <p className="text-xl text-white/90">
                Choose a program that resonates with you, or support all of them to maximize your
